...
sweep_df = 'Z:\UserFolders\ToriArriola\DARPA_updated\PreProcessedData\Pinot\DarpaSweep\Electrode_21and42';
% sweep_df = 'C:\Users\arrio\Box\BensmaiaLab\ProjectFolders\DARPA\Data\ProcessedData\Pinot\DarpaSweep\Electrode_21and42';
...
% wp
  % [~,coeffs, ~,~,~, warn] = FitSigmoid(x_mech, y_mech_dprime,...
   % 'NumCoeffs', 4, 'CoeffInit', [1000,0.02,NaN,NaN], 'EnableBackup', false, 'PlotFit', true);           [~,coeffs, ~,~,~, warn] = FitSigmoid(x_mech, y_mech_pdetect,'NumCoeffs', 2, 'CoeffInit', [.05,.01,NaN,NaN],  'PlotFit', true);
           ...
           [~,coeffs, ~,~,~, warn] = FitSigmoid(x_mech, y_mech_pdetect,'NumCoeffs', 4, 'Constraints', [0.02, 200; -5, 5] , 'PlotFit', true);
%Elect table dt
...
% %mech detect dprime example
% title('Mechanical Detection d''')
% 
% scatter(MechDetect_DT.MechAmp, MechDetect_DT.dPrime, 50, [.1 .1 .1], 'filled')
% % plot(MechDetect_DT.MechAmp, MechDetect_DT.dPrime, 'Color', [.1 .1 .1], 'LineStyle', '-')
% mq = linspace(0, x_mech(end));
...
%elect detect dprime example
title('Sweep pDetect')

plot([0,2],[0,2], 'LineStyle','--','color', [.6,.6,.6])
scatter(predict(1),p_detect_big(2,2),150, 'filled', 'MarkerEdgeColor', rgb(229, 115, 115), 'MarkerFaceColor',rgb(229, 115, 115))
scatter(predict(2),p_detect_big(2,3),150, 'filled', 'MarkerEdgeColor', rgb(229, 57, 53), 'MarkerFaceColor',rgb(229, 57, 53))
scatter(predict(3),p_detect_big(2,4),150, 'filled', 'MarkerEdgeColor', rgb(183, 28, 28), 'MarkerFaceColor',rgb(183, 28, 28))
plot([0 p_detect_big(2,1) p_detect_big(2,1)], [p_detect_big(2,1) p_detect_big(2,1) 0], ...
    'LineStyle','--', 'Color', rgb(136, 14, 79))

% text(2, 2, 'Mech+Elec 19', 'Color',rgb(26, 35, 126), 'FontSize',15)
text(.8, .5, (sprintf('Mech+%.0fuA', tt(up))), 'Color',rgb(183, 28, 28), 'FontSize',20)

% text(2, 1.75, 'Mech+Elec 18', 'Color',rgb(156, 39, 176), 'FontSize',15)
text(.8, .4, (sprintf('Mech+%.0fuA', tt(mm_p))), 'Color',rgb(229, 57, 53), 'FontSize',20)

% text(2, 1.5, 'Mech+Elec 17', 'Color', rgb(103, 58, 183), 'FontSize',15)
text(.8, .3, (sprintf('Mech+%.0fuA', tt(ll_p))), 'Color', rgb(229, 115, 115), 'FontSize',20)
text(.8, .2, 'MechOnly', 'Color',  rgb(136, 14, 79), 'FontSize',20)
...
title('Sweep d''')
scatter(dprime_predicted(2),dprime_big(2,2),150,'filled', 'MarkerEdgeColor', rgb(229, 115, 115), 'MarkerFaceColor',rgb(229, 115, 115))
scatter(dprime_predicted(3),dprime_big(2,3),150,'filled', 'MarkerEdgeColor',rgb(229, 57, 53), 'MarkerFaceColor',rgb(229, 57, 53))
scatter(dprime_predicted(4),dprime_big(2,4),150,'filled', 'MarkerEdgeColor',rgb(183, 28, 28), 'MarkerFaceColor',rgb(183, 28, 28))
plot([0 dprime_big(2,1) dprime_big(2,1)], [dprime_big(2,1) dprime_big(2,1) -1],'LineStyle','--', 'Color', rgb(136, 14, 79))
plot([0,5],[0,5], 'LineStyle','--','color', [.6,.6,.6])
